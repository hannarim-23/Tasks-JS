...
class Library {
  constructor(books = []) {
    // ← если books не передан, то books = []
    this.books = books;
  }
  //a. Выведите в console названия всех книг.
  printAllTitles() {
  ...
  printMostPopularBooks() {
    ...
    let authors = Array.from(new Set(this.books.map((book) => book.author)));

    for (const author of authors) {
      let authorBooks = this.books.filter((book) => book.author === author);
      let maxPop = Math.max(...authorBooks.map((book) => book.popularity));
      let bestBook = authorBooks.find((book) => book.popularity === maxPop);
      console.log(`Самая популярная книга у ${author} - ${bestBook.title} рейтинг ${maxPop}`);
    }
  }
  ...
  printSortedByYear() {
    ...
    let sortBooks = [...this.books].sort((book1, book2) => book1.year - book2.year);
...
const book_1 = new Book('978-5-04-113245-3', 'Евгений Онегин', 'Александр Пушкин', 1833, 'Роман в стихах', 224, 5, 'Мой дядя самых честных правил, когда не в шутку занемог...');
const book_2 = new Book('978-5-17-069842-4', 'Капитанская дочка', 'Александр Пушкин', 1836, 'Исторический роман', 192, 5, 'Береги честь смолоду.');
const book_3 = new Book('978-5-389-01543-2', 'Пиковая дама', 'Александр Пушкин', 1834, 'Повесть', 64, 4, 'Однажды играли в карты у конногвардейца Нарумова.');
const book_4 = new Book('978-5-08-005678-1', 'Руслан и Людмила', 'Александр Пушкин', 1820, 'Поэма', 160, 5, 'У лукоморья дуб зелёный; Златая цепь на дубе том...');
const book_5 = new Book('978-5-446-70012-4', 'Медный всадник', 'Александр Пушкин', 1833, 'Поэма', 48, 4, 'На берегу пустынных волн стоял Он, дум великих полн.');

// --- Марк Твен (5 книг) ---
const book_6 = new Book('978-0-14-310733-0', 'Приключения Тома Сойера', 'Марк Твен', 1876, 'Приключения', 272, 5, '— Том! Нет ответа.');
const book_7 = new Book('978-0-486-28061-5', 'Приключения Гекльберри Финна', 'Марк Твен', 1884, 'Роман', 366, 5, 'Вы обо мне ничего не знаете, если не читали книжки под названием «Приключения Тома Сойера».');
const book_8 = new Book('978-0-19-953664-1', 'Принц и нищий', 'Марк Твен', 1881, 'Исторический роман', 240, 4, 'В старом городе Лондоне в один осенний день родился мальчик в бедной семье по фамилии Кенти.');
const book_9 = new Book('978-1-58049-591-2', 'Янки из Коннектикута при дворе короля Артура', 'Марк Твен', 1889, 'Фэнтези', 416, 4, 'Я американец. Я родился и вырос в Хартфорде, штат Коннектикут.');
const book_10 = new Book('978-0-553-21142-9', 'Жизнь на Миссисипи', 'Марк Твен', 1883, 'Автобиография', 384, 3, 'Миссисипи — река во всех отношениях примечательная.');

// --- Стивен Кинг (5 книг) ---

const book_11 = new Book('978-1-5011-4297-0', 'Оно', 'Стивен Кинг', 1986, 'Ужасы', 1138, 5, 'Ужас, который не кончался еще добрых двадцать восемь лет, начался с того, что мальчик сделал лодочку из газетной страницы.');
const book_12 = new Book('978-1-5011-4351-9', 'Сияние', 'Стивен Кинг', 1977, 'Психологический хоррор', 447, 5, 'Джек Торранс подумал: Самоуверенный сукин сын.');
const book_13 = new Book('978-0-307-74365-7', 'Противостояние', 'Стивен Кинг', 1978, 'Постапокалиптика', 1152, 4, '— Хапс, давай-ка заправим эту колымагу, — сказал Вик.');
const book_14 = new Book('978-1-4447-2344-1', 'Темная Башня I: Стрелок', 'Стивен Кинг', 1982, 'Тёмное фэнтези', 304, 5, 'Человек в черном ушел в пустыню, и стрелок последовал за ним.');
const book_15 = new Book('978-1-5011-5619-9', 'Кэрри', 'Стивен Кинг', 1974, 'Мистика', 199, 4, 'Никто не удивился, когда первый камень попал в цель.');

const books = [book_1, book_2, book_3, book_4, book_5, book_6, book_7, book_8, book_9, book_10, book_11, book_12, book_13, book_14, book_15];

const myLibrary = new Library(books);
myLibrary.printAllTitles();
myLibrary.printMostPopularBooks();
